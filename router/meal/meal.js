...
module.exports = async (req, res) => {
    ...
    startDate = `${startDate.getFullYear()}${formatDate(startDate.getMonth() + 1)}${formatDate(startDate.getDate())}`;
    endDate = `${endDate.getFullYear()}${formatDate(endDate.getMonth() + 1)}${formatDate(endDate.getDate())}`;

    
    const url = `http://open.neis.go.kr/hub/mealServiceDietInfo?MLSV_FROM_YMD=${startDate}&MLSV_TO_YMD=${endDate}&ATPT_OFCDC_SC_CODE=${office_id}&SD_SCHUL_CODE=${school_id}&Type=json&KEY=${key}`;
    ...
    try {
        await request(url, (err, response, mealInfo) => {
            ...
            for (let i = 0; i < mealInfo.length; i++) {
                const mealDate = parseInt(mealInfo[i].MLSV_YMD.substring(6, 8));
                if(mealInfo[i].MMEAL_SC_CODE === "1") {
                    breakfastList[mealDate - startDate.substring(6, 8)] = mealInfo[i].DDISH_NM;
                } else if(mealInfo[i].MMEAL_SC_CODE === "2") {
                    lunchList[mealDate - startDate.substring(6, 8)] = mealInfo[i].DDISH_NM;
                } else if(mealInfo[i].MMEAL_SC_CODE === "3") {
                    dinnerList[mealDate - startDate.substring(6, 8)] = mealInfo[i].DDISH_NM;
                }
            }
